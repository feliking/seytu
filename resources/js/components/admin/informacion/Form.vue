            ...
            <v-row>
              ...
              <v-col cols="12">
                <v-textarea
                  ...
                  label="Descripción"
                  :rules="[v => !!v || 'Requerido', v => (v && v.length <= 2000) || 'No mayor a 2000 caracteres']">
                </v-textarea>
              </v-col>
            </v-row>
          </v-form>
      ...
      try {
        if (this.$refs.form.validate()) {
          if (this.selectedIndex != -1) {
            await axios.put("api/informacion/"+this.selectedItem.id, this.selectedItem)
          } else {
            await axios.post("api/informacion", this.selectedItem)
          }
